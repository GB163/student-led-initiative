...
// ✅ Socket events for MOBILE (user only)
export const socketEvents = {
  // ==================== USER REGISTRATION ====================
  
  // ✅ Register user role - matches web pattern
  registerRole: async (role, userId) => {
    try {
      const socket = await ensureConnected();
      
      const data = {
        role: role,        // Always "user" for mobile
        userId: userId,    // Backend fetches name from DB
        device: 'mobile',
        timestamp: new Date().toISOString()
      };

      console.log('📡 Mobile: Registering role:', data);
  ...
  onUserActivityUpdate: (callback) => {
    try {
      ...
      if (socket) {
        ...
        console.log('👂 Listening for userActivityUpdate events');
  ...
  onOnlineUsersList: (callback) => {
    try {
      ...
      if (socket) {
        ...
        console.log('👂 Listening for onlineUsersList events');
      ...
      console.error('❌ Error setting up onlineUsersList listener:', error);
    }
  },
